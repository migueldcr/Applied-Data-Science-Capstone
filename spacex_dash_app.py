...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        filtered_df_site['class_values']=class_values
        fig = px.pie(filtered_df_site, values='class_values', 
        names='class', 
...
def get_scatter_plot(entered_site,payload_slider):
    if entered_site == 'ALL':
        ...
        data_section = spacex_df[(payload_slider[0]<spacex_df['Payload Mass (kg)']) & (spacex_df['Payload Mass (kg)']<payload_slider[1]) & (spacex_df['Launch Site']==entered_site)]
        fig = px.scatter(data_section, x='Payload Mass (kg)', y='class',
        color="Booster Version Category",title='Correlation between Payload and Success for all sites')
        return fig
